...
int proc(char *tokens[],list histor, char *arg3[]);
...
int showvar(char *tokens[], char *arg3[]);
int changevar(char *tokens[], char *arg3[]);
int showenv(char *tokens[], char *arg3[]);
int fork_cmd(char *tokens[]);
...
int process_comando(char *tokens[],list histor, char *arg3[]){
   char *tok[MAX_TOKENS];
   int h,j=0;
   pos p;
...
int process(char *tokens[],char *arg3[]){
   int i;


   if(strcmp(tokens[0],"changevar")==0){
       return changevar(tokens +1,arg3);
   }else if(strcmp(tokens[0],"showvar")==0){
       return showvar(tokens +1,arg3);
   }else if(strcmp(tokens[0],"showenv")==0){
       return showenv(tokens+1,arg3);
   }
   for(i= 0 ; cmds[i].cmd_name!= NULL ; i++){
...
int proc(char *tokens[],list histor,char *arg3[]){
   if(strcmp(tokens[0],"comando")==0 || strcmp(tokens[0],"hist")==0)
        return process_comando(tokens, histor,arg3);
     else    
        return process(tokens,arg3);
}
